<script >
    $('.formBTNS').on('click', function(e) {
        ...
        checkboxFields.forEach((field) => {
            if (!field.checked) {
                ...
                if (label) {
                    let labelText = label.textContent.replace(/\([^)]*\)/g, '').trim(); // Parantez içindeki metni kaldır
        ...
        if (missingFiles.length > 0 || uncheckedBoxes.length > 0) {
            let message = '';
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (window.useOnlyStudentTabManager === true) {
            console.log("Using only StudentTabManager for step management. scriptF step management is disabled.");
            return;
        }
        ...
        const scholarshipCompletionModal = new bootstrap.Modal(document.getElementById(
            'scholarshipCompletionModal'));
...
<script>
    function getFormData(step) {
        const formData = {};
        document.querySelectorAll(`#step-${step} input, #step-${step} select, #step-${step} textarea, #step-${step} checkbox`).forEach(input => {
...
<script !src="">
    modalCloseButton.addEventListener("click", () => {
        scholarshipCompletionModal.hide();
        window.location.href = '/student/Kayit-Yenileme-Tamamla';
    });
</script>
<script !src="">
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.forEach(function(district) {
                    ...
                    option.value = district.isim; // district.id ile değiştirilmiş ID değeri
                    option.text = district.isim; // district.isim ile ilçe ismi
                    option.setAttribute('data-id', district.id);  // Fakülte id'sini data-id olarak ekle
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (p_school_city !== null) {
            ...
            p_school_city.addEventListener('change', function() {
                ...
                var selectedOption = p_school_city.options[p_school_city.selectedIndex]; // Seçili option
                var dataId = selectedOption.getAttribute('data-id'); // Seçili option'ın data-id'sini al
        ...
        if (p_school_district !== null) {
            ...
            p_school_district.addEventListener('change', function() {
                ...
                var selectedOption = p_school_district.options[p_school_district.selectedIndex]; // Seçili option
                var dataId = selectedOption.getAttribute('data-id'); // Seçili option'ın data-id'sini al
        ...
        if (m_school_city !== null) {
            ...
            m_school_city.addEventListener('change', function() {
                ...
                var selectedOption = m_school_city.options[m_school_city.selectedIndex]; // Seçili option
                var dataId = selectedOption.getAttribute('data-id'); // Seçili option'ın data-id'sini al
        ...
        if (m_school_district !== null) {
            ...
            m_school_district.addEventListener('change', function() {
                ...
                var selectedOption = m_school_district.options[m_school_district.selectedIndex]; // Seçili option
                var dataId = selectedOption.getAttribute('data-id'); // Seçili option'ın data-id'sini al
        ...
        if (h_school_district !== null) {
            ...
            h_school_district.addEventListener('change', function() {
                ...
                var selectedOption = h_school_district.options[h_school_district.selectedIndex]; // Seçili option
                var dataId = selectedOption.getAttribute('data-id'); // Seçili option'ın data-id'sini al
        ...
        if (h_school_city !== null) {
            ...
            h_school_city.addEventListener('change', function() {
                ...
                var selectedOption = h_school_city.options[h_school_city.selectedIndex]; // Seçili option
                var dataId = selectedOption.getAttribute('data-id'); // Seçili option'ın data-id'sini al
