                        ...
                        <form>
                            ...
                            <div class="step-content" id="step-3" data-content="3">
                                ...
                                <div class="row">
                                        ...
                                        <select onchange="getCities(this,'current_university');" id="university_city" class="form-select">
                                            <option selected disabled > Sehir Seciniz</option>
                                            @foreach($unicities as $city)
                                                <option data-id="{{ $city->sehir }}"  @if( isset($aday->educinfo->university_city) && $aday->educinfo->university_city == $city->sehir) selected @endif value="{{$city->sehir}}">{{$city->sehir}}</option>
                                            @endforeach
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class='form-select' onchange="getFaculties(this,'faculty');" name='current_university' id='current_university'>
                                               <option selected disabled> Şehir Seçiniz...</option>
                                                    @if(isset($aday->educinfo) && !is_null($aday->educinfo->current_university))
                                                        <option selected value="{{ $aday->educinfo->current_university }}">{{ $aday->educinfo->current_university }}</option>
                                                    @endif
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select onchange="getDepartmants(this,'departmant');"  name='faculty' id="faculty" class="form-select">
                                            <option value="">Seçiniz...</option>
@if(isset($aday->educinfo) && !is_null($aday->educinfo->faculty))
    <option selected value="{{ $aday->educinfo->faculty }}">{{ $aday->educinfo->faculty }}</option>
@endif

                                        </select>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="departmant">Bölüm</label>
                                        <select  name='departmant' id="departmant" class="form-select">
                                            <option value="">Seçiniz...</option>
                                                @if(isset($aday->educinfo) && !is_null($aday->educinfo->grade_departmant))
                                                        <option selected value="{{ $aday->educinfo->grade_departmant }}">{{ $aday->educinfo->grade_departmant }}</option>
                                                    @endif
                                        ...
                                        <select id="universityStatus" class="form-select">
                                            ...
                                            <option @php if(isset($aday->educinfo) && isset($aday->educinfo->university_type) && $aday->educinfo->university_type == 'Vakıf'){ echo "selected";} @endphp value="Vakıf">Vakıf</option>
                                            <option @php if(isset($aday->educinfo) && isset($aday->educinfo->university_type) && $aday->educinfo->university_type == 'Devlet'){ echo "selected";} @endphp value="Devlet">Devlet</option>
                                        </select>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="universityStatus">Öğrenim Türü</label>
                                        <select id="universityStatus" class="form-select">
                                            <option selected disabled >Seçiniz...</option>
                                            <option @selected(isset($aday->educinfo->university_type) && $aday->educinfo->university_type == 'Örgün') value="Örgün">Örgün</option>
                                            <option @selected(isset($aday->educinfo->university_type) && $aday->educinfo->university_type == 'İkinci Öğretim') value="İkinci Öğretim">İkinci Öğretim</option>
                                            <option @selected(isset($aday->educinfo->university_type) && $aday->educinfo->university_type == 'Uzaktan') value="Uzaktan">Uzaktan</option>
                                            <option @selected(isset($aday->educinfo->university_type) && $aday->educinfo->university_type == 'AçıkÖğretim') value="AçıkÖğretim">AçıkÖğretim</option>

                                        </select>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="educationYear">Yeni Öğretim Yılında Kaçıncı Sınıfta
                                            Olacaksınız?</label>
