<script>
    ...
    const url = "{{ route('burs-taksitleri.edit', ['id' => ':id']) }}".replace(':id', candidateId);
...
<script>
    ...
    $(document).ready(function() {
                ...
                render: function(data, type, row) {
                    if (type === 'display' && column.visible === true) {
                        switch(column.name) {
                            ...
                            case 'burs_tipi_adi':
                                return row.burs_tipi_adi || '';
                            case 'donem':
                                return row.donem || '';
                            case 'burs_tutari':
                                return row.burs_tutari != null && row.burs_tutari !== '' ? Number(row.burs_tutari).toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '';
                            case 'taksit_sayisi':
                                return row.taksit_sayisi != null ? row.taksit_sayisi : '';
                            case 'baslangic_tarihi':
                                return row.baslangic_tarihi ? normalizeToDMY(row.baslangic_tarihi) : '';
                            default:
        ...
        dtColumns.push({
            ...
            searchable: false,
            render: function(data, type, row) {
                var url = "{{ route('burs-taksitleri.edit', ['id' => ':id']) }}".replace(':id', row.id);
                if (type === 'display') {
                    return `
        ...
        table = $('#bankTable').DataTable({
            ...
            stateSave: true,
            ajax: {
                url: "{{ route('burs-taksitleri.data') }}",
                type: 'GET',
                data: function(d) {
                    d.filters = filters;
...
<script>
    $(document).ready(function() {
        function submitForm(redirectUrl = null) {
            ...
            // AJAX isteği
            $.ajax({
                url: '{{route('burs-taksitleri.store')}}',
                type: 'POST',
                data: {
                    formData: formData,
                ...
                error: function(xhr, status, error) {
                    iziToast.error({
                        ...
                        message: 'Ssitemde kayıtlı olan veriyi girdiniz. Yeni Kayıt Deneyiniz!',
                    });
