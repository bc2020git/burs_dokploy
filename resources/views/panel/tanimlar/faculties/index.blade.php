...
@endsection
        ...
        <main class="main-content px-3 py-4">
            <div class="mt-1">
                <div class="navbar-button-container">
                    ...
                    <div class="navbar-button-container">
                        <button type="button" class="btn" id="bulkEdit">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                                <path d="M15.0252 13.8477C14.3723 12.1792 14.9993 10.7467 14.9993 9.16699C14.9993 5.94408 12.3889 3.33366 9.16602 3.33366C5.9431 3.33366 3.33268 5.94408 3.33268 9.16699C3.33268 12.3899 5.9431 15.0003 9.16602 15.0003C10.7457 15.0003 12.1782 14.3732 13.2283 13.3542L13.3533 13.2293Z" fill="#fff"/>
                            </svg>
                            Toplu Düzenle
                        </button>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="bulkEditModalLabel">Fakülte Üniversite Düzenle</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body w-100">
                        <div class="form-group">
                            <label for="universitySelect">Üniversite Seçin</label>
                            <select class="form-control" id="universitySelect" name="university_id">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" id="confirmBulkEdit">Kaydet</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    @section('scripts')
        ...
        <script>
            ...
            function loadUniversities() {
                $.ajax({
                    ...
                    success: function(response) {
                        const select = $('#universitySelect');
                        select.empty();
                        select.append('<option value="">Üniversite Seçin</option>');
            ...
            // Toplu düzenleme butonu click eventi
            $('#bulkEdit').on('click', function() {
                ...
                if (checkboxes.length === 0) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Uyarı',
                        text: 'Lütfen düzenlenecek fakülteleri seçin!',
            ...
            $('#confirmBulkEdit').on('click', function() {
                ...
                if (!universityId) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Uyarı',
                        text: 'Lütfen bir üniversite seçin!',
                        confirmButtonText: 'Tamam'
                    });
                    return;
                }
                ...
                checkboxes.forEach(checkbox => {
                    const facultyId = checkbox.getAttribute('data-id');
                    if (facultyId) {
                        selectedIds.push(facultyId);
                    }
                });

                $.ajax({
                    url: '{{ route("bulk.update.faculties") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Başarılı',
                                text: 'Fakülteler başarıyla güncellendi',
                                confirmButtonText: 'Tamam'
                            }).then((result) => {
