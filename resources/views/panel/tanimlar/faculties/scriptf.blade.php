<script>
    ...
    const url = "{{ route('edit-faculties', ['id' => ':id']) }}".replace(':id', candidateId);
...
<script>
    ...
    $(document).ready(function() {
                    ...
                    if (type === 'display' && column.visible === true) {
                        switch(column.name) {
                            ...
                            case 'univercity.type':
                                return getType(row.univercity.type);
        ...
        dtColumns.push({
            ...
            searchable: false,
            render: function(data, type, row) {
                var url = "{{ route('edit-faculties', ['id' => ':id']) }}".replace(':id', row.id);
                if (type === 'display') {
                    return `
        ...
        table = $('#facultiesTable').DataTable({
            ...
            buttons: [
                ...
                },
                {
                    ...
                    title: 'Fakülteler Listesi',
                    exportOptions: {
                        columns: [5, 6, 2], // Sadece: Fakülte Kodu, Fakülte Adı, Üniversite Adı
                        rows: function(idx, data, node) {
                            const selectedRows = $('.checkbox:checked');
                            if (selectedRows.length > 0) {
                                return $(node).find('.checkbox').prop('checked');
