<script>
    ...
    const url = "{{ route('edit-univercities', ['id' => ':id']) }}".replace(':id', candidateId);
...
<script>
    ...
    $(document).ready(function() {
        ...
        function getType(item) {
                if(item == 'Vakıf/Özel') return '<span class="status-box-vakif">Vakıf/Özel</span>';
        ...
        dtColumns.push({
            ...
            searchable: false,
            render: function(data, type, row) {
                var url = "{{ route('edit-univercities', ['id' => ':id']) }}".replace(':id', row.id);
                if (type === 'display') {
                    return `
        ...
        table = $('#universitiesTable').DataTable({
            ...
            stateSave: true,
            ajax: {
                url: "{{ route('get-univercities.data') }}",
                type: 'GET',
                data: function(d) {
                    d.filters = filters;
            ...
            buttons: [
                {
                    extend: 'excel',
                    title: 'Üniversiteler Listesi',
                    exportOptions: {
                        columns: [1,2,3,4,5,6,7,8,9], // Tüm veriler (checkbox ve işlemler hariç)
                ...
                },
                {
                    ...
                    title: 'Üniversiteler Listesi',
                    exportOptions: {
                        columns: [1, 2, 3, 4], // Sadece: Kod, Ad, Tür, Şehir
                        rows: function(idx, data, node) {
                            const selectedRows = $('.checkbox:checked');
                            if (selectedRows.length > 0) {
                                return $(node).find('.checkbox').prop('checked');
