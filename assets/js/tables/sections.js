document.addEventListener("DOMContentLoaded", function () {
  ...
  sections.forEach((section, index) => {
    ...
    const actionButtonHTML =
      ...
      section.plakaNo === "04" ||
      section.plakaNo === "06"
        ? `  <button class="btn edit-section-info-btn" data-index="${index}" onclick="location.href='/panel/Bolum-Detay'"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
...
const rowInnerHTML = `
  ...
  <td>${section.universiteKodu}</td>
  <td>${section.universiteAdi}</td>
  <td><span class="${getBadgeClass(section.universiteTuru)}">${section.universiteTuru}</span></td>
  <td>${section.uniSehir}</td>
  <td>${section.fakulteKodu}</td>
  <td>${section.fakulteAdi}</td>
  <td>${section.programKodu}</td>
  <td>${section.programAdi}</td>
  <td><span class="${getBadgeClass(section.ogretimSuresi)}">${section.ogretimSuresi}</span></td>
  <td><span class="${getBadgeClass(section.ogretimTuru)}">${section.ogretimTuru}</span></td>
  <td><span class="${getBadgeClass(section.puanTuru)}">${section.puanTuru}</span></td>
  <td class="bursDurumuOnlisans"><span class="${getBadgeClass(section.bursVeriliyorMuOnlisans)}">${section.bursVeriliyorMuOnlisans}</span></td>
  <td class="bursDurumuLisans"><span class="${getBadgeClass(section.bursVeriliyorMuLisans)}">${section.bursVeriliyorMuLisans}</span></td>
  <td class="bursDurumuYlisans"><span class="${getBadgeClass(section.bursVeriliyorMuYlisans)}">${section.bursVeriliyorMuYlisans}</span></td>
  <td class="bursDurumuDoktora"><span class="${getBadgeClass(section.bursVeriliyorMuDoktora)}">${section.bursVeriliyorMuDoktora}</span></td>
...
row.addEventListener("click", function () {
  window.location.href = "/panel/Bolum-Detay";
});

// Tıklama etkisini durdurma
row
  .querySelectorAll("td:nth-child(14),td:nth-child(1), input[type='checkbox']")
  .forEach((cell) => {
