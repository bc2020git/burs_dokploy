...
switch ($scholar->form->status) {
 ...
 public function addConfirmedScholar($id){
        ...
        if($aday->status != 3){
            ...
            if($check && $check->form->first() && !$check->form->first()->infos){
...
public function kayitYenilemeSonuclandir($scholarid,$status){
    ...
    function transferRenewToActive($renewModelClass, $activeModelClass, $scholar_id)
    {
    ...
    function doctransferRenewToActive($renewModelClass, $activeModelClass, $scholar_id)
    {
        // İlgili scholar'ı bul
        $scholar = Scholar::find($scholar_id);
